...
export default function Category() {
  ...
  const subcategories = [
    { name: 'T-shirt', type: 'T-shirt', image: require('../assets/tshirt-sample.png') },
  ...
  const fetchWardrobe = useCallback(async () => {
    try {
      ...
      // Only fetch if we're in a subcategory view (has specific type parameter)
      if (!params.type || params.type === 'Tops') {
        // Main category page - no need to fetch
        setLoading(false);
        return;
      }
      
      // Don't redirect - just try to fetch, show empty if no token
      if (!token) {
      ...
      if (!response.ok) {
        ...
        console.log('Failed to fetch wardrobe items, showing empty state');
        setItems([]);
        setLoading(false);
        return;
      }
      ...
      // Fix: Use data.items instead of data directly
