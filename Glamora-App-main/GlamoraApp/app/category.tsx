...
export default function Category() {
  ...
  const fetchCustomSubcategories = useCallback(async () => {
    try {
      ...
      const response = await fetch(API_ENDPOINTS.subcategories('Tops'), {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.bodyBackground }]}>
      ...
      <View style={[styles.headerContainer, { backgroundColor: theme.colors.headerBackground }]}>
        ...
        <Text style={[styles.categoryTitle, { color: theme.colors.headerText }]}>{categoryType === 'T-shirt' ? 'T-shirts' : categoryType}</Text>
        <TouchableOpacity 
